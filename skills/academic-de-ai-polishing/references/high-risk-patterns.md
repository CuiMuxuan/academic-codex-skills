...
| Forced subsection bridging | Every subsection ends by announcing or justifying the next subsection | The prose connects sections because it feels required, not because a real relation exists. | Delete false bridges. If a real relation exists, name the relation specifically; otherwise let subsections stand on their local purpose. |
| Abrupt future preview | A paragraph invokes a later method, result, concept, or conclusion before it has been introduced | The reader is asked to accept a later idea without setup. | Move the point later, add the missing setup, or reduce it to brief structure navigation. |
| Undefined abstraction | "framework", "mechanism", "strategy", "feature", or "performance" appears before the exact referent is clear | The sentence sounds scholarly but the object is vague. | Name the referent, define the scope, or replace the abstraction with the concrete operation, variable, result, or relation. |
...
| Duplicate subsection explanation | Neighboring subsections repeat the same background, definition, or evaluation in different words | The text feels padded and weakens local section identity. | Keep the strongest version, merge the overlap, or turn the repetition into an explicit contrast if the distinction matters. |
