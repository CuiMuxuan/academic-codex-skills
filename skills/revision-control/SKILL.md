...
- `scripts/migrate_bilingual_titles.py`: adds or updates `title_en`, `title_zh`, `bilingual_title`, `title_translation_status`, and `title_translation_source` for paper, chapter, and section objects from a JSON title map. Use it when an existing object library or rebuild migration lacks bilingual title fields.
